...
$$\text{sample mean} = \bar{X}_n \approx Normal\left(m, \frac{s}{\sqrt{n}}\right) \approx Normal\left(\hat{m}_n, \frac{\hat{s}}{\sqrt{n}}\right) $$
...
$$\hat{s}_n Z + \hat{m}_n = Normal\left(\hat{m}_n, \frac{\hat{s}}{\sqrt{n}}\right)$$

### probability $\hat{m}_n$ is in the CI
$$P\left(\text{low} \le Normal\left(\hat{m}_n, \frac{\hat{s}}{\sqrt{n}}\right) \le \text{high}\right)$$
$$= P\left(\frac{\text{low} - \hat{m}_n}{\hat{s}_n} \le Z \le\frac{\text{high} - \hat{m}_n}{\hat{s}_n}\right)$$
...
## Formula: To change the $pct$% CI

a. Find the low quantile $z^*$
$$P\left(Z \le -z^*\right) = pct/2$$
...
$$\hat{m}_n = \hat{p}_n = \frac{\text{successes}}{n} = \frac{37}{120}, \quad \frac{\hat{s}}{\sqrt{120}} = \frac{\sqrt{\hat{p}(1-\hat{p})}}{\sqrt{120}} $$
